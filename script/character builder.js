...
if (!window.indexedDB) {
    ...
    console.log(lang[currentLanguage][118]);
    ...
    FUDB.open = function(onSuccessFn=null) {
        ...
        request.onupgradeneeded = function(event) {
            ...
            console.log(lang[currentLanguage][119])
        }
        request.onsuccess = function(event) {
            FUDB.db = event.target.result;
            console.log(lang[currentLanguage][120]);
        ...
        request.onerror = function(event) {
            console.log(lang[currentLanguage][121] + event.target.error);
        }
    };
    ...
    class tabManager{
        ...
        add(item) {
            ...
            request.onsuccess = function(e) {
                console.log(lang[currentLanguage][122]);
            }
            request.onerror = function(e) {
                console.log(lang[currentLanguage][123]+item.id);
            }
        ...
        update(item) {
            ...
            request.onsuccess = function(e) {
                console.log(lang[currentLanguage][124]);
            }
            request.onerror = function(e) {
                console.log(lang[currentLanguage][123]+item.id);
            }
        ...
        get(idRecord, onSuccessFn, onErrFn = null) {
            ...
            request.onerror = function(e) {
                console.log(lang[currentLanguage][125] + "'" + idRecord + "'");
                if (this.onErrFn) {
                    this.onErrFn();
                }
            }
        ...
        keys(onSuccessFn){
            ...
            request.onsuccess = function(e) {
                if(e.target.result.length>0){
                    console.log(lang[currentLanguage][126]+"'"+e.target.source.name+"'.");
                }

                this.onSuccessFn(e.target.result);
            }
            request.onerror = function(e) {
                console.log(lang[currentLanguage][127]+this.tabName);
            }
        }
        ...
        delete(idRecord, onSuccessFn) {
            ...
            request.onsuccess = function(e) {
                console.log("'"+idRecord+"': " + lang[currentLanguage][129]);
                this.onSuccessFn();
            }
            request.onerror = function(e) {
                console.log(lang[currentLanguage][128]+"'"+idRecord+"'");
            }
        }
    }
...
function resetCache() {
    if (window.confirm(lang[currentLanguage][130])) {
        if (window.confirm(lang[currentLanguage][131])) {
            FUDB.reset();
        }
    }
}
...
function createNewCharacter() {
	...
	newPC.peculiarity = {
        pecName: lang[currentLanguage][132],
        pecDesc: lang[currentLanguage][133]
    }
	
	newPC.zeroPowerProgress = 0
	
	newPC.zeroPower = {
        zeroName: lang[currentLanguage][134],
        zeroTrigger: lang[currentLanguage][135],
        zeroEffect: lang[currentLanguage][136],
    }
...
function setupEvents() {
    ...
    document.getElementById("confirmObjectToEdit").addEventListener("click", 
        function(e){
            if(document.getElementById("confirmObjectToEdit").innerHTML==lang[currentLanguage][69]){
                ...
                document.getElementById("confirmObjectToEdit").innerHTML=lang[currentLanguage][137];
                ...
                if(list=="classes" || list=="items"){
                    ...
                    imgHeader.innerHTML=lang[currentLanguage][138];
            ...
            } else if(document.getElementById("confirmObjectToEdit").innerHTML==lang[currentLanguage][137]){
                resetEditorMenu();
            }
        }
    );
    document.getElementById("duplicateObject").addEventListener("click", 
        function(e){
            ...
            if (window.confirm(lang[currentLanguage][139]+"'"+document.getElementById("listToEdit").value+"'?")){
    ...
    document.getElementById("deleteObject").addEventListener("click", 
        function(e){
            ...
            if (window.confirm(lang[currentLanguage][140]+"'"+document.getElementById("listToEdit").value+"'?")){
                if(item.refObj.hasOwnProperty("arr") && item.refObj.hasOwnProperty("idx")){
    ...
    let toIconClosed=document.querySelectorAll("[setExpander='0']")
    let toIconOpen=document.querySelectorAll("[setExpander='1']")

    for(let g=0;g<toIconClosed.length;g++){
        toIconClosed[g].appendChild(expandIcon());
    }

    for(let h=0;h<toIconOpen.length;h++){
        toIconOpen[h].appendChild(expandIcon(false));
    }
    ...
    document.getElementById("shareChar").addEventListener("click", function(){
		if(p2p.peer){
			...
			if (listOfChars.value) {
				...
				if(window.confirm(lang[currentLanguage][141]+"'"+listOfChars.value+"'?")){
			...
			alert(lang[currentLanguage][145])
		}
	});
    ...
    document.getElementById("shareLib").addEventListener("click", function(){
		if(p2p.peer){
			...
			if (libName!=null) {
				...
				if(window.confirm(lang[currentLanguage][146]+"'"+libName+"'?")){
					FUDB.libraries.get(libName,
						...
						function() { //se non si trova il personaggio, viene restituito un messaggio di errore
							console.log(lang[currentLanguage][147]);
						}
					)
				}
			} else {
    ...
    document.getElementById("kickUser").addEventListener("click", function(){
		if(p2p.peer && p2p.master){
			...
			if (p2p.connList.filter(c=>c.peer===userID).length>0) {
				//si presuppone una sola libreria per ogni nome
				if(window.confirm(lang[currentLanguage][148]+"'"+p2p.connList.filter(c=>c.peer===userID)[0].userName+"'?")){
					kickUser(userID)
				}
			} else {
				console.log(lang[currentLanguage][149]);
			}
		} else {
			alert(lang[currentLanguage][145])
		}
    ...
    document.getElementById("pecName").addEventListener("click", (e)=>{editPeculiarity(e, lang[currentLanguage][132], "pecName")});
    document.getElementById("pecDesc").addEventListener("click", (e)=>{editPeculiarity(e, lang[currentLanguage][133], "pecDesc")});
	
    //evento di form cambio potere Zero
    document.getElementById("zeroName").addEventListener("click", (e)=>{editZeroPower(e, lang[currentLanguage][134], "zeroName")});
    document.getElementById("zeroTrigger").addEventListener("click", (e)=>{editZeroPower(e, lang[currentLanguage][135], "zeroTrigger")});
    document.getElementById("zeroEffect").addEventListener("click", (e)=>{editZeroPower(e, lang[currentLanguage][136], "zeroEffect")});
	...
	//eventi slot equip: si indicano la lista di oggetti equipaggiabili nello slot su cui si è cliccato il menu contestuale
    ...
    document.getElementById("ipIcon").firstChild.popupContent = lang[currentLanguage][154];
...
function createRoller(params={}){
	...
	if(typeof params.type!=undefined){
		if(params.type=="stats"){
			pz1.innerHTML=
				"<option value='"+"des"+"'>"+lang[currentLanguage][7]+"</option>   \
				<option value='"+"int"+"'>"+lang[currentLanguage][8]+"</option>   \
				<option value='"+"vig"+"'>"+lang[currentLanguage][9]+"</option>   \
				<option value='"+"vol"+"'>"+lang[currentLanguage][10]+"</option>"
		}
	}
	...
	buttonForChat.innerHTML=lang[currentLanguage][155];
...
//funzione che mostra il popup (se nascosto) e ne cambia la posizione per riflettere il punto in cui è stato cliccato il mouse. Il
//contenuto del popup dipende dalla proprieta' "popupContent" dell'elemento cliccato a cui l'evento è associato
function showPopup(e) {
    if(e.target.hasOwnProperty("popupContent")){
        ...
        if(e.target.popupContent instanceof HTMLElement){
            ...
            document.getElementById("popupContent").innerHTML = (e.target.popupContent ? e.target.popupContent : lang[currentLanguage][156]);
        }
...
//funzione per modificare la peculiaritè
...
function updateValue(newValue, targetEl, defaultValue="...", path=[]) {
    //check: se il nome è nullo o composto da spazi, si sostituisce con "..."
    if (newValue.trim() == "") {
...
function addMod(trg, whichStat) {
    ...
    trg.innerHTML = lang[currentLanguage][157];
...
function addClass(e) {
    ...
    newRow.innerHTML = lang[currentLanguage][158];
    ...
    data.classes.forEach(function(className) {
        if (!characterInfo.levels[className]) { //se la classe ancora non è stata selezionata, si aggiunge alla select
            let el = document.createElement("option");
            el.value = className;
            el.innerHTML = className;
            list.appendChild(el);
        } else if (characterInfo.levels[className] < data.maxLv) { //se la classe invece è gia' stata aggiunta in passato e non ha ancora raggiunto il lv massimo consentito, si conta come non padroneggiata
            notMastered++;
        }
    });

    //se il numero totale di classi non padroneggiate è inferiore a 3, si consente di andare avanti e mostrare la select per scegliere
    //una nuova classe, altrimenti si esce (non è consentito avere piu' di tre classi non padroneggiate allo stesso tempo)
    if (notMastered < data.maxClasses) {
        ...
        document.getElementById("addNewClass").style.display = "none"; //si fa sparire temporaneamente il tasto per l'inserimento di classe, almeno finchè non viene completata la selezione in populateClass
...
function addBond(e) {
    ...
    if (!document.getElementById(toBeId)) {
        ...
        newBox[0].value = lang[currentLanguage][159];
        newBox[0].id = toBeId;
...
function addItem() {
    ...
    data.items.forEach(function(sourceEl) {
        ...
        confirmButton.title = lang[currentLanguage][160];
...
function addSkill() {
    ...
    if (Object.keys(characterInfo.levels).length > 0) {
        ...
        characterInfo.skills.forEach(function(skl) {
            if (skl.TIPO == lang[currentLanguage][168]) {
                countMasteries--;
            }
        });
        ...
        data.abilities.forEach(function(el) {
            ...
            //check sul livello attuale dell'abilita' vs il livello massimo che puo' avere - se è gia' al massimo, non viene visualizzata
            ...
            if (reqSklIndex && !skip && Number(characterInfo.skills[reqSklIndex].MAX_LA) > 1) { //si assume che TUTTE le abilita' con MAX_LA>1 debbano essere prese piu' volte per sbloccare incantesimi aggiuntivi
                ...
                if (boundedSkillsNbr >= Number(characterInfo.skills[reqSklIndex].LA) + (hasAbility(characterInfo, lang[currentLanguage][170]+"(" + (el.CLASSE.toUpperCase()) + ")") ? 2 : 0)) {
                    skip = true;
                }
            }
            ...
            if (!skip) {
                if (el.TIPO == lang[currentLanguage][171] && hasLevels(characterInfo, el.CLASSE)) {
                    if (el.REQUISITI != "") {
                        if (!hasAbility(characterInfo, el.REQUISITI)) {
                            skip = true;
                        }
                    }
                } else if (el.TIPO == lang[currentLanguage][172] && (hasLevels(characterInfo, el.CLASSE) || hasAbility(characterInfo, lang[currentLanguage][170]+"(" + (el.CLASSE.toUpperCase()) + ")")) || (hasAbility(characterInfo, lang[currentLanguage][173]) && el.REQUISITI == lang[currentLanguage][173]) || (hasAbility(characterInfo, lang[currentLanguage][174]) && el.REQUISITI == lang[currentLanguage][174])) {
                    let count = 0;
                    ...
                    if (hasAbility(characterInfo, lang[currentLanguage][170]+"(" + (el.CLASSE.toUpperCase()) + ")")) {
                        count = count + 2;
                    }
                ...
                } else if (el.TIPO == lang[currentLanguage][168] && (hasLevels(characterInfo, el.CLASSE)==data.maxLv || el.CLASSE=="")) {
                    if (denyHeroics)
                        skip = true;
                } else {
                    skip = true;
            ...
            //se non è stato trovato motivo di skip, allora si aggiunge nella lista l'abilita' esaminata
            if (!skip) {
        ...
        document.getElementById("availableAbilities").innerHTML = lang[currentLanguage][169];
    }
}
...
function populateClass(cls = null) {
    ...
    //se la funzione è stata invocata da evento, allora si inizializza nei dati in memoria i dati della classe e dei benefici correlati
    if (cls instanceof HTMLElement) {
        selectedClass = cls.value;
        //se la classe viene creata per la prima volta e non popolata da memoria - quindi è originata da evento
        if (!characterInfo.levels[selectedClass]) {
            ...
            for (let i = 0; i < data.abilities.length; i++) {
                if (selectedClass == data.abilities[i].CLASSE && data.abilities[i].TIPO == lang[currentLanguage][175]) {
    ...
    } else { //la classe scelta è gia' settata in memoria, quindi viene popolata la scheda con gli elementi corrispondenti
...
function populateBond(bondContainer, tokenRow = null) {
    ...
    //se il nome del legame non è vuoto o composto da spazi, si crea il legame in scheda
    if (bondName.trim() != "") {
        ...
        newBond.appendChild(createCombo(bondContainer, 0, ["", lang[currentLanguage][176], lang[currentLanguage][177]]));
        newBond.appendChild(createCombo(bondContainer, 1, ["", lang[currentLanguage][178], lang[currentLanguage][179]]));
        newBond.appendChild(createCombo(bondContainer, 2, ["", lang[currentLanguage][180], lang[currentLanguage][181]]));
        ...
        //se non esistono sentimenti collegati,vuol dire che è un legame nuovo: pertanto, si inizializzano i valori e si caricano nel PG in memoria
        if (!bondContainer.feelings) {
            let bondObject = {
                name: bondName,
                feelings: [0, 0, 0]
...
function populateItem(item) {
    ...
    removalButton.addEventListener("click", function(f) {
        if (window.confirm(lang[currentLanguage][182])) {
            removeFromInventory(f.target.itemToRemove); //si passa l'oggetto da rimuovere alla funzione apposita
            updateSheet();
        }
    });

    //si crea la tag per segnalare se l'oggetto è eventualmente equipaggiato
    if (item.equipped) {
...
function populateSkill(skill) {
    ...
    //se l'abilita' non è un beneficio (che sono legati alla classe e non possono essere rimossi), si aggiunge anche l'icona per l'eliminazione della skill
    if (skill.TIPO != lang[currentLanguage][175]) {
        ...
        if(newEntry.refObj.LA<=1){
            ...
            removalButton.addEventListener("click", function(f) {
                if (window.confirm(lang[currentLanguage][183])) {
...
function editLibrary(editorContent){
    if(uploadEditorValues(editorContent)){
        alert(lang[currentLanguage][186]+"'"+editorContent.refObj.NOME+"'.")
        setLibraryToBeSaved(true);
        populateEditorSelect();
    } else {
        alert(lang[currentLanguage][185]+"'"+editorContent.refObj.NOME+"'.")
    }
    resetEditorMenu();
}

function resetEditorMenu(){
    document.getElementById("libraryEditor").innerHTML=lang[currentLanguage][184]
    document.getElementById("selectListToEdit").disabled = false;
    document.getElementById("listToEdit").disabled = false;
    document.getElementById("confirmObjectToEdit").innerHTML=lang[currentLanguage][69];
}
    ...
    for (let i = 0; i < characterInfo.skills.length; i++) {
        ...
        if (skillName === characterInfo.skills[i].NOME) {
            if (characterInfo.skills[i].LA > 1) {
                ...
                //si controllano tutte le altre abilita' e, se è stata appena rimossa l'abilita' requisito, vengono rimosse anche queste in modo iterativo
                let j = 0;
...
function calculateDefense(chr) {
    ...
    Object.keys(chr.equip).forEach(function(item) {
        if (chr.equip[item]) {
            //se il tipo difesa dell'oggetto è "BASE", allora il numero in MOD_DIF è il valore base di difesa, altrimenti è un bonus rispetto al valore di DES base
            ...
            if (chr.equip[item].TIPO_DIF == lang[currentLanguage][211]) {
                baseDef = Number(chr.equip[item].MOD_DIF);
            } else if (chr.equip[item].TIPO_DIF == lang[currentLanguage][220]) {
...
function calculateMagicDefense(chr) {
    ...
    Object.keys(chr.equip).forEach(function(item) {
        if (chr.equip[item]) {
            //se il tipo difesa dell'oggetto è "BASE", allora il numero in MOD_DIF è il valore base di difesa, altrimenti è un bonus rispetto al valore di INT base
            ...
            if (chr.equip[item].TIPO_DIF_MAG == lang[currentLanguage][211]) {
                baseDef = Number(chr.equip[item].MOD_DIFM);
            } else if (chr.equip[item].TIPO_DIF_MAG == lang[currentLanguage][220]) {
...
function maxHp(chr) {
    ...
    chr.skills.forEach(function(skl) {
        if (skl.MOD_PV.indexOf("|") == -1) {
            skillBonus += Number(skl.MOD_PV)*(skl.LA?skl.LA:1); //se MOD_PV è un numero e non una formula del tipo (20:10|40)
        } else {
        ...
        if(chr.equip[it]){
            if(chr.equip[it].AB_OGGETTO){
                for(let x=0;x<chr.equip[it].AB_OGGETTO.length;x++){
                    ...
                    if (skl.MOD_PV.indexOf("|") == -1) {
                        itemSklBonus += Number(skl.MOD_PV)*(skl.LA?skl.LA:1); //se MOD_PV è un numero e non una formula del tipo (20:10|40)
                    } else {
                        let formula = skl.MOD_PV.split("|");
                        if (characterLevel(chr) >= Number(formula[0].split(":")[0])) {
...
function maxMp(chr) {
    ...
    chr.skills.forEach(function(skl) {
        if (skl.MOD_PM.indexOf("|") == -1) {
            skillBonus += Number(skl.MOD_PM)*(skl.LA?skl.LA:1); //se MOD_PM è un numero e non una formula del tipo (20:10|40)
        } else {
...
function maxIp(chr) {
    ...
    chr.skills.forEach(function(skl) {
        if (skl.MOD_PI.indexOf("|") == -1) {
            skillBonus += Number(skl.MOD_PI)*(skl.LA?skl.LA:1); //se MOD_PM è un numero e non una formula del tipo (20:10|40)
        } else {
...
function hasCompetency(item) {
    let result = true;
    if (item.MARZIALE == "M") {
        if (item.RAGGIO == lang[currentLanguage][187] && !characterInfo.martial.melee) {
            result = false;
        } else if (item.RAGGIO == lang[currentLanguage][188] && !characterInfo.martial.range) {
            result = false;
        } else if (item.CATEGORIA == lang[currentLanguage][189] && !characterInfo.martial.armor) {
            result = false;
        } else if (item.CATEGORIA == lang[currentLanguage][190] && !characterInfo.martial.shield) {
            result = false;
        }
    }
    return result;
...
//verifica se il personaggio è in crisi (PV<=meta' PV max) e cambia le classi di alcuni elementi per visualizzarlo in scheda
...
function updateSheet(chr = characterInfo) {
    //effettua autosalvataggio se il flag è attivo
    if(chr.autosave){
        saveCharacter(true);
    }
    ...
    if(hasAbility(characterInfo, lang[currentLanguage][191])){
    ...
    if (!chr.equip.main) { //se non c'è un'arma assegnata alla mano principale, viene equipaggiata l'arma in indice 0 (Colpo Senz'Armi) della variabile "data", parametro "default"
        chr.equip.main = JSON.parse(JSON.stringify(data.items[0]));
    }
    ...
    if (!chr.equip.secondary) { //vedi sopra, si equipaggia il Colpo Senz'Armi
        document.getElementById("secondaryHand").innerHTML = lang[currentLanguage][192];
    ...
    if (!chr.equip.armor) { //vedi sopra, si equipaggia l'oggetto 1 - armatura "Nessuna"
        chr.equip.armor = JSON.parse(JSON.stringify(data.items[1]));
    }
    ...
    if (!chr.equip.accessory) {
        document.getElementById("accessory").innerHTML = lang[currentLanguage][192];
    ...
    if (!chr.equip.bonusAccessory) {
        document.getElementById("bonusAccessory").innerHTML = lang[currentLanguage][192];
    ...
    document.getElementById("ip_max").innerHTML = lang[currentLanguage][193]+"<span>" + chr.ip.max + "</span>";
    ...
    if (chr.skills) {
        ...
        chr.skills.forEach(function(el) {
            //si definisce la testata rappresentante la classe sotto la quale l'abilita' va messa, creandola se non esiste
            if (el.CAPACITA == "") { //se non è un incantesimo o una abilita' similare
                if (!skillContainers.hasOwnProperty(el.CLASSE)) { //se skillContainers non contiene gia' la classe dell'abilita' esaminata, si crea un nuovo raggruppamento sotto cui appendere le finestre abilita' strutturate
                    skillContainers[el.CLASSE] = createClassHeader(el.CLASSE);
        ...
        Object.keys(chr.levels).forEach(function(cls) {
            ...
            if (chr.levels[cls] > compareVal) {
                if (memoText == "") {
                    memoText = lang[currentLanguage][194]
                } else {
                    memoText += ", ";
                }
                memoText += cls + ": " + (chr.levels[cls] - compareVal) + lang[currentLanguage][195];
            }

            //i livelli sono meno delle abilita' scelte
            if (chr.levels[cls] < compareVal) {
                if (memoText == "") {
                    memoText = lang[currentLanguage][196]
                } else {
                    memoText += ", ";
                }
                memoText += cls + ": " + (compareVal - chr.levels[cls]) + lang[currentLanguage][195];
            }
        });
...
function startEquip(ev) {
    ...
    //si identifica l'elemento draggato con un attributo, perchè l'implementazione del drag n drop fa cacare
...
function checkEquippable(slot, item){
    //si verifica se l'arma è marziale e, nel caso, se il personaggio ha competenza in quel tipo di arma
    let skip = !hasCompetency(item);

    if (!skip) {
        //se l'oggetto è gia' equipaggiato, si disequipaggia prima
        //in base allo slot che ha lanciato questo evento, si assegna l'oggetto allo slot giusto
        if (slot == "armor") { //equip corpo
            if (item.MANI == lang[currentLanguage][197] && item.CATEGORIA == lang[currentLanguage][25]) {
				return true
            }
        } else if (slot == "accessory" || slot == "bonusAccessory") { //equip accessorio
            if (item.MANI == lang[currentLanguage][197] && item.CATEGORIA == lang[currentLanguage][27]) {
				return true
            }
        } else if (slot == "secondaryHand" || slot == "secondary") { //equip mano secondaria
            let verify = false;

            if (item.MANI == lang[currentLanguage][198]) {
                verify = true;
            } else if (item.MANI == lang[currentLanguage][199] && (hasAbility(characterInfo, lang[currentLanguage][200]) && [lang[currentLanguage][202], lang[currentLanguage][203], lang[currentLanguage][204], lang[currentLanguage][205]].includes(item.CATEGORIA))) {
                if (hasAbility(characterInfo, lang[currentLanguage][201])) {
                    if (characterInfo.equip.main.MANI == lang[currentLanguage][199]) {
                        if ([lang[currentLanguage][202], lang[currentLanguage][203], lang[currentLanguage][204], lang[currentLanguage][205]].includes(characterInfo.equip.main.CATEGORIA)) {
                            verify = true;
                        }
                    } else {
                        verify = true;
            ...
            } else if (item.MANI == lang[currentLanguage][206]) {
                if (characterInfo.equip.main) {
                    if (characterInfo.equip.main.MANI == lang[currentLanguage][199]) {
                        if (hasAbility(characterInfo, lang[currentLanguage][200])) {
                            if (hasAbility(characterInfo, lang[currentLanguage][201])) {
                                if ([lang[currentLanguage][202], lang[currentLanguage][203], lang[currentLanguage][204], lang[currentLanguage][205]].includes(characterInfo.equip.main.CATEGORIA)) {
                                    verify = true;
                                }
            ...
            return verify
        } else if ((slot == "mainHand" || slot == "main") && (item.MANI == lang[currentLanguage][206] || item.MANI == lang[currentLanguage][199] || (item.MANI == lang[currentLanguage][198] && hasAbility(characterInfo, lang[currentLanguage][207])))) { //equip mano principale
            return true
        }
	} else {
		return false
...
function equip(s, it = null) {
    ...
    //si passa il riferimento all'oggetto e si usa per fare tutti i passaggi successivi (l'oggetto in inventario è lo stesso che viene equipaggiato, non c'è cloning!)
    if (it) {
        item = it;
    } else {
	...
	if(checkEquippable(slot, item)){
		...
		//controllo per disequipaggiare se la mano secondaria è incompatibile con la mano primaria
		if(slot == "mainHand" || slot == "main"){
			if (item.MANI == lang[currentLanguage][206]) {
				if (characterInfo.equip.secondary) {
					if (item.CATEGORIA != characterInfo.equip.secondary.CATEGORIA && !hasAbility(characterInfo, lang[currentLanguage][201]) && characterInfo.equip.secondary.MANI !=lang[currentLanguage][198]) {
						unequip("secondary", false);
					}
				}
			} else if (item.MANI == lang[currentLanguage][199]) {
				//se c'è un oggetto nella mano secondaria, si verifica se le abilita' possedute consentono a quell'oggetto di rimanere equipaggiato
				if (characterInfo.equip.secondary) {
					let checkIfUnequipSec = true;

					if (hasAbility(characterInfo, lang[currentLanguage][200]) && [lang[currentLanguage][202], lang[currentLanguage][203], lang[currentLanguage][204], lang[currentLanguage][205]].includes(item.CATEGORIA)) {
						if (hasAbility(characterInfo, lang[currentLanguage][201])) {
							if ([lang[currentLanguage][202], lang[currentLanguage][203], lang[currentLanguage][204], lang[currentLanguage][205]].includes(characterInfo.equip.secondary.CATEGORIA)) {
...
function addInventoryContainer(existingContainer = null) {
    ...
    emptyButton.addEventListener("click", function(f) {
        if (window.confirm(lang[currentLanguage][208])) {
...
function addNewNote(refObj=null) {
    ...
    if(refObj==null){
        ...
        removalButton=fetchSvgIcon("trash");
        removalButton.addEventListener("click", function(f) {
            if (window.confirm(lang[currentLanguage][209])) {
...
function renameItemContainer(name, header, targetEl) {
    ...
    if (name.trim() != "" && document.querySelectorAll("[containerName=\"" + name + "\"]").length == 0) {
        ...
        newContextMenuRow.innerHTML="<span>"+lang[currentLanguage][210]+"'"+name+"'</span>";
...
function buildItem(i) {
    ...
    if (i.CATEGORIA == lang[currentLanguage][189] || i.CATEGORIA == lang[currentLanguage][190]) {
        body.innerHTML += "<div style='border-bottom: solid 1px black;'>" + i.CATEGORIA + isMartial + " - " + i.COSTO + "Z</div>";
        body.innerHTML += "<div>"+lang[currentLanguage][19]+": " + ((i.TIPO_DIF != lang[currentLanguage][211] && i.MOD_DIF.charAt(0) != "-") ? "+" : "") + i.MOD_DIF + ", "+lang[currentLanguage][20]+": " + ((i.TIPO_DIF != lang[currentLanguage][211] && i.MOD_DIFM.charAt(0) != "-") ? "+" : "") + i.MOD_DIFM + ", "+lang[currentLanguage][18]+": " + i.MOD_INIZ + "</div>";
    } else if (i.CATEGORIA != lang[currentLanguage][27]) {
    ...
    body.innerHTML += "<div>"+lang[currentLanguage][212]+": " + encodeHTML(i.QUALITA) + "</div>";
    body.style.display = "none";
    body.style.flexDirection = "column";
...
function buildAbility(a) {
    ...
    header.innerHTML = (a.TIPO == lang[currentLanguage][168] ? "["+lang[currentLanguage][168]+"] " : "") + (a.TIPO == lang[currentLanguage][172] ? lang[currentLanguage][172]+": " : "") + (a.OFFENSIVO == "x" ? '[⚡] ' : "") + a.NOME;
    ...
    body.innerHTML += "<div style='border-bottom: solid 1px black;'> " + (Number(a.MAX_LA) > 1 ? lang[currentLanguage][213] + a.MAX_LA + (a.LA ? ", "+lang[currentLanguage][214]+": " + a.LA : "") : "") + "</div>";

    //il testo cambia a seconda della natura dell'abilita'
	let dat="";
	if(a.COSTO.trim()!=""){
		dat += lang[currentLanguage][215]+": " + a.COSTO + ", ";
	}
	if(a.BERSAGLIO.trim()!=""){
		dat += dat!=""?", ":"";
		dat += lang[currentLanguage][216]+": " + a.BERSAGLIO + ", ";
	}
	if(a.DURATA.trim()!=""){
		dat += dat!=""?", ":"";
		dat += lang[currentLanguage][217]+": " + a.DURATA;
	}
    ...
    if (a.REQUISITI != "") {
        body.innerHTML += "<div style='border-bottom: solid 1px black;'>"+lang[currentLanguage][218]+": " + a.REQUISITI + "</div>";
    }
...
function applySpecialNotation(html) {
    ...
    for (let la of html.getElementsByClassName("la")) {
        if (html.refObj.LA) {
            la.innerHTML = la.innerHTML.replace(lang[currentLanguage][219], "<span style='color:green;font-weight:bolder'>" + html.refObj.LA + "</span>")
        }
    }
}
...
function openEditor(obj, callbackFunction, targetContainer=null) {
    ...
    //se l'oggetto è un oggetto, lo si riconosce se possiede la proprieta' "RARITA", non posseduta dalle abilita'
    let setOfTags = null;
    ...
    Object.keys(setOfTags).forEach(function(propName) {
        if(setOfTags.hasOwnProperty(propName)){
            ...
            //se l'elemento da creare è una select, si riempiono le varie opzioni, altrimenti si indica il tipo di input da assegnare all'elemento
            if (prop.tag == "select") {
                let bitOption = document.createElement("option");
    ...
    button.innerHTML = lang[currentLanguage][221]
...
function itemFilters() {
    ...
    //durante l'iterazione, si verifica se è gia' stato valutato quel valore: se no, si pusha nell'array della proprieta' esaminata
    ...
    //filtro per il nome
    filterName.innerHTML = "<span style='margin-right:1em;'>"+lang[currentLanguage][222]+"</span>";
    ...
    filterCategory.innerHTML = "<span style='margin-right:1em;'>"+lang[currentLanguage][223]+"</span>";
    ...
    filterRarity.innerHTML = "<span style='margin-right:1em;'>"+lang[currentLanguage][224]+"</span>";
    ...
    filterRange.innerHTML = "<span style='margin-right:1em;'>"+lang[currentLanguage][225]+"</span>";
    ...
    filterMartial.innerHTML = "<span style='margin-right:1em;'>"+lang[currentLanguage][226]+"</span>";
    ...
    filterHands.innerHTML = "<span style='margin-right:1em;'>"+lang[currentLanguage][227]+"</span>";
    ...
    filterDmgType.innerHTML = "<span style='margin-right:1em;'>"+lang[currentLanguage][228]+"</span>";
...
function applyItemFilter(e) {
    ...
    for (let item of itemCollection) {
        ...
        Object.keys(filter).forEach(function(filterName) {
            ...
            //se l'attributo è assegnato, si prende la stringa in lowercase prima di passarla ai filtri, altrimenti si restituisce la stringa vuota
    ...
    for (let i = 0; i < allWrappers.length; i++) {
        if (allWrappers[i].querySelectorAll(".hidden[nome]").length == allWrappers[i].querySelectorAll("[nome]").length) { //si confrontano il numero degli elementi con attributo "nome" aventi la classe .hidden con il loro numero totale: se coincidono, allora il wrapper è vuoto e va nascosto
...
function createNewClock(params={}, callbackOnChange=null){
	...
	valueLabel.innerHTML=lang[currentLanguage][229]
	maxLabel.innerHTML=lang[currentLanguage][230]
...
function createTracker(score, editable=true, leVar=null) {
    ...
    const headName = {
        fabula: { tag: lang[currentLanguage][231], icon_id: "book", icon_class: "falt", icon_src: "book-open", icon_fn: null },
        hp: { tag: lang[currentLanguage][232], icon_id: "heart", icon_class: "falt", icon_src: "heartbeat", icon_fn: e => heal() },
        mp: { tag: lang[currentLanguage][233], icon_id: "mind", icon_class: "falt", icon_src: "brain", icon_fn: e => refresh() },
        ip: { tag: lang[currentLanguage][234], icon_id: "ipIcon", icon_class: "falt", icon_src: "briefcase", icon_fn: null },
        clock: { tag: lang[currentLanguage][235], icon_id: "", icon_class: "falt", icon_src: "clock", icon_fn: null }
    }

    //se il punteggio indicato è valido
    if (headName[score]) {
        ...
        if (newPart.refObj.max) {
			...
			if(editable){
				...
				foot.title=lang[currentLanguage]["26t"];
		...
		newPart.textForChat={
			get value(){
				...
				text+="\n"+lang[currentLanguage][236]+": "+targetScore.target.value+(targetScore.target.max?"/"+targetScore.target.max:"")
    ...
    FUDB.chars.get(characterInfo.name,
        function(result) { //se si trova il personaggio, viene eseguita la funzione onSuccessFn di aggiornamento dati
            ...
            if (auto) {
                ...
                doSave=window.confirm(lang[currentLanguage][237]);
            }
...
async function deleteCharacter() {
	if(window.confirm(lang[currentLanguage][238])){
		let listOfChars=document.getElementById("listOfChars");
		if (listOfChars.value) {
			...
			console.log(lang[currentLanguage][239]);
		}
    }
}
...
async function fetchCharFromMemory() {
    ...
    if (listOfChars.value) {
        ...
        FUDB.chars.get(listOfChars.value,
            function(result) { //se si trova il personaggio, viene eseguita la funzione onSuccessFn di caricamento dati
                console.log(lang[currentLanguage][240]);
                let cleanData=sanitizeString(result.data)
                let loadedData = JSON.parse(cleanData);
                loadCharacter(loadedData);
                console.log(lang[currentLanguage][241]);
            },
            function() { //se non si trova il personaggio, viene restituito un messaggio di errore
                console.log(lang[currentLanguage][242]);
            }
        );
    } else {
        console.log(lang[currentLanguage][243]);
    }
}
    ...
    if(document.getElementById("listOfLibraries").value){
        ...
        FUDB.chosenSet.get("default",
            function(result) { //se si trova la libreria, viene eseguita la funzione onSuccessFn di aggiornamento dati
                ...
                alert(lang[currentLanguage][254]+defaultLib+".")
                console.log(lang[currentLanguage][255])
            },
            function() { //se non si trova la libreria, viene eseguita la funzione onErrorFn di inserimento dati
                ...
                alert(lang[currentLanguage][254]+defaultLib+".")
                console.log(lang[currentLanguage][256])
            }
        );
    }
}
...
async function loadDefaultLibrary() {
    FUDB.chosenSet.get("default",
        ...
        function() { //se non si trova la libreria, viene eseguita la funzione onErrorFn di inserimento dati
            console.log(lang[currentLanguage][257])
        }
    );
}
...
function saveLibraryPrompt(){
    getLibrary().id= window.prompt(lang[currentLanguage][258],getLibrary().id)
    saveLibrary();
}

//funzione per restituire il set giusto di libreria da gestire
function getLibrary(){
    let dataToSave={}

    if(typeof fullData.id!=="undefined"){
        dataToSave=fullData
    } else {
        dataToSave=data
    }
...
//funzione per settare propriamente le librerie
function setLibrary(library){
    //libreria id+lingue
    if(typeof library.maxLv==="undefined"){
        fullData = library;
        data = library[currentLanguage];
    } else {
        //libreria mono lingua
        fullData = {};
        data = library;
    }
}
...
async function saveLibrary() {
    let dataToSave=getLibrary()

    //si presuppone una sola libreria per ogni nome
    if(dataToSave.hasOwnProperty("id")){
        FUDB.libraries.get(dataToSave.id,
            function(result) { //se si trova la libreria, viene eseguita la funzione onSuccessFn di aggiornamento dati
                if (window.confirm(lang[currentLanguage][237])) {
                    FUDB.libraries.update({ id: dataToSave.id, data: JSON.stringify(dataToSave) })
                    setLibraryToBeSaved(false);
                }
            },
            function() { //se non si trova la libreria, viene eseguita la funzione onErrorFn di inserimento dati
                FUDB.libraries.add({ id: dataToSave.id, data: JSON.stringify(dataToSave) });
...
async function loadLibrary(loadedData) {
    /*if(typeof loadedData[currentLanguage]!=="undefined"){
        fullData = loadedData;
        data = fullData[currentLanguage];
    } else {
        data = loadedData;
    }*/
    setLibrary(loadedData)
	addChatLine("Info", lang[currentLanguage][259]+loadedData.id+".")
    document.getElementById("curLibraryName").innerHTML="<b>"+loadedData.id+"</b>"
    updateSheet();
}
	...
	if(window.confirm(lang[currentLanguage][238])){
		...
		if (listOfLibs.value) {
			...
			FUDB.libraries.delete(listOfLibs.value, 
				function(){
					let idToDelete=getLibrary().id;
					FUDB.chosenSet.get("default",
						function(result) { //se si trova la libreria, viene eseguita la funzione onSuccessFn di aggiornamento dati
							if(idToDelete==result.data){
								FUDB.libraries.delete("default", 
									function(){
										console.log(lang[currentLanguage][245])
									}
								);
							}
						},
			...
			console.log(lang[currentLanguage][261]);
		}
    }
}
...
function downloadLibrary() {
    ...
    var file = new Blob([JSON.stringify(dataToSave)], { type: 'text/plain' });
    a.href = URL.createObjectURL(file);
    a.download = dataToSave.id + '.fudb';
    a.click();
}
        ...
        FUDB.libraries.get(libName,
            function(result) { //se si trova la libreria, viene eseguita la funzione onSuccessFn di caricamento dati
				...
				if(characterInfo.inventory.length>0 || characterInfo.skills.length>0){
					if (window.confirm(lang[currentLanguage][262])) {
						checkCharacterUpdate()
					}
				}
            },
            function() { //se non si trova il personaggio, viene restituito un messaggio di errore
                console.log(lang[currentLanguage][263]);
            }
        );
    } else {
        console.log(lang[currentLanguage][263]);
			...
			Object.keys(inv).forEach(
				(prop)=>{
					if(inv[prop]!==lookUp[0][prop] && (typeof inv[prop]!="undefined" && typeof lookUp[0][prop]!="undefined" ) && (typeof inv[prop]!="object" && typeof lookUp[0][prop]!="object" )){
						if (window.confirm(lang[currentLanguage][264]+"'"+lookUp[0].NOME+"', "+lang[currentLanguage][265]+" '"+prop+"': "+lang[currentLanguage][266]+" '"+inv[prop]+"', "+lang[currentLanguage][267]+" '"+lookUp[0][prop]+"'. "+lang[currentLanguage][268])) {
	...
	if(!data[document.getElementById("selectListToEdit").value]){
		if(data.editorTags[document.getElementById("selectListToEdit").value.toUpperCase()]){
			...
			Object.keys(data.editorTags[document.getElementById("selectListToEdit").value.toUpperCase()]).forEach(
				(prop)=>{
					if(prop==="NOME"){
						tempEntry[prop]=lang[currentLanguage][244]
					} else {
						tempEntry[prop]=""
					}
				}
...
function setLibraryToBeSaved(flag){
    if(flag){
        document.getElementById("curLibraryName").innerHTML="<b style='color:red;'>"+getLibrary().id+" - "+lang[currentLanguage][246]+"</b>" 
        document.getElementById("saveLib").style.removeProperty("display");
    } else {
        document.getElementById("curLibraryName").innerHTML="<b>"+getLibrary().id+"</b>" 
				...
				if(p2p.peer == null || (p2p.peer != null && p2p.master)){
					...
					if(roll.rolls.filter(r=>r.max != null).length==2 && roll.rolls.filter(r=>r.value==roll.rolls.reduce((partialSum, a) => partialSum + a.value, 0)/2).length==2){
						...
						if(roll.rolls.filter(r=>r.value>=6).length==2){
							critTxt="<span style='color:green;'>["+sender+"] "+lang[currentLanguage][246]+"</span>"
						} else if(roll.rolls.filter(r=>r.value==1).length==2){
							critTxt="<span style='color:red;'>["+sender+"] "+lang[currentLanguage][247]+"</span>"
						} else if(hasAbility(characterInfo, lang[currentLanguage][249])){
							critTxt="<span style='color:lime;'>["+sender+"] "+lang[currentLanguage][248]+"</span>"
						}
...
function changeLanguage(language){
    let changeLang=FUDB.lang.update({ id: "0", data: language })

    changeLang.onsuccess= ()=>{
        refreshLang(language)
...
function refreshLang(language){
    let langTags=document.querySelectorAll("[langtag]")
    let langIdx=""
    let newStr=""
    ...
    for(let i=0;i<langTags.length;i++){
        langIdx=langTags[i].getAttribute("langtag")
        newStr=lang[language][langIdx]

        if(newStr!=""){
            langTags[i].innerHTML=newStr
        }

        if(langTags[i].getAttribute("title")!==undefined){
            if(lang[language][langIdx+"t"]!==undefined){
                langTags[i].setAttribute("title", lang[language][langIdx+"t"])
            }
        }
    }

    document.getElementById("languagesMenu").value=language
}
...
function setupLang(){
    ...
    document.getElementById("languages").addEventListener("change", 
        (e)=>{
            refreshLang(e.target.value)
        }
    );

	document.getElementById("langChoser").style.removeProperty("display")
	...
	FUDB.open(
		()=>{
			FUDB.lang.get("0", 
				(res)=>{
					document.getElementById("langChoser").style.display="none";
                    refreshLang(res.data)
                    setupEULA()
				},
				()=>{
					document.getElementById("langChoser").firstElementChild.style.removeProperty("display")
				}
			)
		}
	);
...
function selectLang(id="languages"){
	document.getElementById("langChoser").style.display="none";
    let lang=document.getElementById(id).value
	let addRes=FUDB.lang.add({ id: "0", data: lang });
    addRes.onsuccess= ()=>{
        refreshLang(lang)
        setupEULA()
    }
...
function setupEULA(){
	...
	FUDB.open(
		()=>{
			FUDB.EULA.get("0", 
				()=>{
                    console.log(lang[currentLanguage][252])
					document.getElementById("EULA").style.display="none";
					startUp()
				},
				()=>{
                    console.log(lang[currentLanguage][251])
...
function acceptEULA(){
	let adding=FUDB.EULA.add({ id: "0", data: "1" });
    adding.onsuccess= ()=>{
        console.log(lang[currentLanguage][253])
        setupEULA()
    }
}
...
function startUp() {
    ...
    setLibrary(defaultData); //inizializzazione dei dati usati dalla scheda - di default si associano i dati contenuti nel file "data.js"
	//fullData = defaultData; //inizializzazione dei dati usati dalla scheda - di default si associano i dati contenuti nel file "data.js"
